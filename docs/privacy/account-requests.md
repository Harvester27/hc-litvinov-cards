...
3. V projektu Firebase `lancers-web-cards-2026` vyhledat ověřené UID. Před odstraněním Auth účtu odstranit odpovídající dokumenty `lancersCardsPlayers/{uid}` a `tipovackaPreview/{uid}` a všechny dokumenty v `comments`, kde `userId == uid`, včetně skrytých komentářů (`isDeleted == true`).
